...
void ExodusModel::formAuxiliary() {
    ...
    if (mHasStaceyABC && mElementalVariables_axis.at("edge_aspect_ratio").maxCoeff() > 1.5) {
        handleMeshRefinementForABC();
    }
...
void ExodusModel::setInnerBoundary() {
    ...
    for (int i = 0; i < getNumQuads(); i++) {
        ...
        maxdiff(0) = std::max({diff0(0), diff1(0), diff2(0), diff3(0)}); // s or theta
        maxdiff(1) = std::max({diff0(1), diff1(1), diff2(1), diff3(1)}); // z or r
    }
...
void ExodusModel::handleMeshRefinementForABC() {
    std::vector<int> refinementNodeIds;
    for (int i = 0; i < getNumQuads(); i++) {
        if (getSideAxis(i) >= 0 || getSideSurface(i) >= 0 || getSideRightB(i) >= 0 || getSideLowerB(i) >= 0 ) {
            continue;
        }
        for (int j = 0 ; j < 4; j++){
            int node = mConnectivity(i, j);
            double rcoord = isCartesian() ? mNodalS(node) : mNodalTheta(node);
            if (mMeshEdgeCorner(0) - rcoord < tinyDouble) {
                refinementNodeIds.push_back(node);
            }
        }
    }
}
...
void ExodusModel::makeABField(RDCol2 inner) {
    ...
    for (int i = 0; i < getNumQuads(); i++) {
        ...
        RDCol2 centerCrds = (crds0 + crds1 + crds2 + crds3) / 4;
        
        if ((centerCrds(0) > inner(0)) && (centerCrds(1) < inner(1))) {
            mABfield(i) = 3; // corner
        } else if (centerCrds(0) > inner(0)) { // right bry
            mABfield(i) = 1;
        } else if (centerCrds(1) < inner(1)) {
